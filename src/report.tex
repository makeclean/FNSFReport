...
\begin{table}[h!]
	...
	\begin{tabular}{ |c|c|c|c| }
		...
		\hline
		{IB assembly} & 3.51 $\pm$ 0.14\% & 2.83 $\pm$ 0.28\% & 6.34 $\pm$ 0.15\% \\
		\hline
		{OB assembly} & 9.95 $\pm$ 0.10\% & 9.03 $\pm$ 0.17\% & 18.98 $\pm$ 0.10\% \\
		\hline
	\end{tabular}
\end{table}
\begin{figure}[h!]
...
In step 1 of the SDDR calculation workflow the neutron flux was calculated over a mesh covering one sector. the mesh used is 6.02 x 5.73 x \SI{5.98}{cm} (x, y, z directions) and a mapping is shown in figure \ref{fig:N flux mapping}. 

\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.2]{../plots/SDDR_nmap_xy.png}
  \caption{Neutron flux mapping in one sector for SDDR calculations "vertical cross section"}
  \label{fig:N flux mapping}
\end{figure}

\subsection{Decay Heat: results} \label{Decay Heat}
Table \ref{photon heating} gives the results of decay gamma heating in one sector at different times following the shutdown of the facility obtained using DAG-MCNP5 heating tally (F6). The source intensity at each time step was calculated using ALARA and was used as a tally multiplier and the values are given in the table also for each time step.
